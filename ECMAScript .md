...
案例5： 对象的`Symbol.replace`属性，指向一个方法，当该对象被`String.prototype.replace`方法调用时，会返回该方法的返回值。·
...
- 内部抛出错误，则async函数返回值为一个失败的Promise对象。
...
- Object.values()方法返回一个给定对象的所有可枚举属性值的数组，类似于Object.keys()，只是前者返回属性值，后者返回键值组合的数组。
